...
// Apps List
add_shortcode('apps_icon', 'theme_shortcode_apps_icon');
function theme_shortcode_apps_icon($atts, $content = null, $code) {
	extract(shortcode_atts(array(
		'count' 	=> '3',
		'featured'	=> false,
		'cats'		=> false,
	...
	$apps = get_posts( $args );
	$list = '<ul class="apps-icon-list">';
	$counter = 0;
	foreach ( $apps as $app ) {

		$link = get_permalink( $app->ID );
		$resized_icon_src = theme_get_image( get_post_meta($app->ID, 'info_icon', true), 64, 64, true );

		$list .= '<li><a href="' . $link . '"><img src="' . $resized_icon_src . '" /></a></li>';
	}
	$list .= '</ul>';
	return <<<RET
$list
